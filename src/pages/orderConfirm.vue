<template>
  <div class="order-confirm">
        ...
        <div class="order-box">
          <div class="item-address">
            ...
            <div class="addr-list clearfix">
              <div class="addr-info" v-for="(item,i) in list" :key="i" :class="{ checked: i === checkIndex }"
                @click="checkIndex = i">
    ...
    <modal title="新增确认" btnType='1' :showModal='showEditModal' @cancel="closeModal" @submit="submitAddress">
      <template v-slot:body>
        <div class="edit-wrap">
          ...
          <div class="item">
            <el-cascader :options="cityData" v-model="checkedItem.cityModel" :placeholder='checkedItem.placeholder'
              @change="saveCity" :key="isResourceShow" ref="casRef">
            </el-cascader>
          </div>
          <div class="item">
    ...
    <modal title="删除确认" btnType='1' :showModal='showDelModal' @cancel="closeModal" @submit="submitAddress">
      <template v-slot:body>
        <p>您确认要删除此地址吗？</p>
      </template>
    </modal>
...
export default {
  ...
  data () {
    return {
      ...
      cityData, // city 数据
      isResourceShow: 0 // 重新渲染 解决length报错
    }
  },
  components: {
    Modal,
  ...
  methods: {
    // change事件给 区级 加默认值
    saveCity () {
      if (this.checkedItem.cityModel.length === 2) {
        this.checkedItem.cityModel[2] = ''
      }
    },
    // 禁止body 滚动
    stopMove () {
    ...
    Move () {
      ...
      document.removeEventListener('touchmove', func, { passive: true })
    },
    // 查询收获地址
    getAddressList () {
      this.axios.get('/shippings').then((res) => {
        ...
        this.list.forEach(item => {
          item.cityModel = [item.receiverProvince, item.receiverCity, item.receiverDistrict]
        })
      })
    },
    // 打开新增地址弹框
    openAddressModal () {
      ++this.isResourceShow
      this.userAction = '0'
      this.checkedItem = {} // 关键(每次打开新增地址的模态框保证是全空的状态)
      this.checkedItem.placeholder = '请选择收获地址'
    ...
    editAddress (item) {
      ...
      if (this.checkedItem.cityModel[2] === '') {
        this.checkedItem.cityModel.splice(2, 1)
      }
      this.checkedItem.placeholder = this.checkedItem.cityModel.join('/')
      this.userAction = '1'
      this.showEditModal = true
      this.stopMove()
    },
    ...
    submitAddress () {
      ...
      if (userAction === '0' || userAction === '1') {
        ...
        const [receiverProvince, receiverCity, receiverDistrict] = checkedItem.cityModel
        params = {
          receiverName,
          receiverMobile,
          receiverProvince,
        ...
        const regZip = /[1-9]\d{5}(?!\d)/
        if (!receiverName) {
          errMsg = '请输入收货人名称'
        } else if (!receiverMobile || !regMobile.test(receiverMobile)) {
          errMsg = '请输入正确格式的手机号'
        } else if (!receiverProvince || !receiverCity) {
          errMsg = '请选择收获地址'
        } else if (!receiverAddress) {
          errMsg = '请输入详细地址'
          // } else if (!receiverAddress || !receiverDistrict) {
          //   errMsg = '请输入收货地址'
        } else if (!receiverZip || !regZip.test(receiverZip)) {
          errMsg = '请输入正确格式的邮编'
        }
        if (errMsg) {
